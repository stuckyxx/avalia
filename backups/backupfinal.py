...
def calcular_indices_por_secao(respostas, matriz_perguntas):
    ...
    for secao, perguntas in matriz_perguntas.items():
        pontos_possiveis_secao, pontos_obtidos_secao = 0, 0
...
def gerar_relatorio_detalhado_pdf(respostas, municipio, segmento, matriz_perguntas, tipo_relatorio):
    ...
    doc.add_paragraph()
    texto_adicional = (f"Com base na Lei 12.527/2011 (Lei de Acesso à Informação), o nosso controle de qualidade fez uma avaliação geral da {segmento} Municipal de {municipio}, na qual, apresentou as seguintes informações:")
    p_adicional = doc.add_paragraph(texto_adicional); p_adicional.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    doc.add_paragraph()
        ...
        for secao, perguntas in matriz_a_usar.items():
            ...
            for item in perguntas:
                ...
                for subcriterio in item["subcriterios"]:
                    ...
                    p_sub.add_run(f"{subcriterio}: "); run_status = p_sub.add_run(status)
